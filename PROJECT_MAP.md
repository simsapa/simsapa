...
#### `/bridges/assets/qml/` - QML User Interface Components
...
- `bridges/assets/qml/tst_*.qml` - QML component tests
- `bridges/assets/qml/com/profoundlabs/simsapa/` - type definition dummies for qmllint
...
- **Dictionary Inclusion-Set Filtering:** `SearchParams.dict_source_uids: Option<Vec<String>>` carries the per-dict checkbox / lock selection assembled by `bridges/assets/qml/SuttaSearchWindow.qml::compute_dict_search_filter()`. ContainsMatch and HeadwordMatch push `dict_label IN (set)` down via JOIN to `dict_words` (rides `dict_words_dict_label_idx`); Fulltext pushes it into Tantivy via `add_dict_filters`; the dispatcher's `apply_dict_source_uids_filter` is a safety net that drops only `table_name == "dict_words"` rows (DPD-native `dpd_headwords` / `dpd_roots` rows pass through unchanged — the bridge's `dpd_enabled` gate is what protects Combined from leaks). DPD Lookup is structurally DPD-only and ignores user-dict membership by design.
...
- **Snippet & highlight pipeline / "Show All Snippets":** `backend/src/highlight.rs` (`merge_ranges`/`wrap_ranges`/`literal_ranges`, producer-owned non-nested `<span class='match'>`), `query_task.rs` (`results_page` exclusion pass, Contains multi-snippet, `highlight_row` fallback) and `searcher.rs` (`render_snippet`, `expand_doc_occurrences`/`enumerate_match_ranges` Fulltext per-occurrence expansion). UI/header-dedup/find-jump in `bridges/assets/qml/FulltextResults.qml` + `SuttaSearchWindow.qml`; punctuation-tolerant find in `src-ts/find.ts`. Full design: [docs/search-snippet-highlight-pipeline.md](./docs/search-snippet-highlight-pipeline.md).
...
- **Content Display:** QML views in `bridges/assets/qml/`
...
- **User-imported StarDict entry rendering:** `backend/src/app_data.rs::render_dict_word_html` injects `DICTIONARY_CSS`/JS (plus the dictionary's `res/` CSS/JS) at `</head>`, the theme class at `<html>` and the word heading at `<body>`. StarDict `h` entries come as full documents (whitney-gd) or bare fragments (nyanatiloka-gd, peu-gd, reader.dict), so the stored HTML first goes through `html_content::ensure_html_document`, which adds whichever of those tags are missing (an entry with no tags at all is plain text and goes through `html_content::plain_text_to_html`: blank-line blocks → `<p>`, single line breaks → `<br>`, existing entities kept) — without it a fragment renders unstyled (white background) with no double-click lookup. Applied at render time, so already-imported dictionaries need no re-import.
...
- **Download Interface:** `cpp/download_appdata_window.cpp`, `bridges/assets/qml/DownloadAppdataWindow.qml`
...
- **Gloss Tab:** `bridges/assets/qml/GlossTab.qml` - Pali text analysis with vocabulary and AI translations
  ...
  - **AI Translation Interface:** `bridges/assets/qml/AssistantResponses.qml` - Tabbed interface for multiple AI model responses. Diffs the incoming entries array against an internal `ListModel` (per-row `setProperty` patches; full reset only on a length / request_id-sequence change) so response updates never rebuild the delegates, and emits `tabSelectionChanged` only from a real tab-button click. **Height note:** the selected response renders as RichText and the `StackLayout` height is driven by a `content_height` property the inner item pushes up via `Layout.onPreferredHeightChanged` (not a one-shot `itemAt()` binding, which isn't reactive to late RichText `contentHeight` and truncated restored sessions).
  - **AI Request Coordinator:** `bridges/assets/qml/AiResponseCoordinator.qml` - Shared non-visual component (one instance per tab in `GlossTab.qml` / `PromptsTab.qml`) owning AI request entry construction, `request_id` generation and stale-response fencing, retry-by-index, and per-request cancellation. See [docs/ai-model-management-and-fallback.md](./docs/ai-model-management-and-fallback.md).
  - **Response Tab Buttons:** `bridges/assets/qml/ResponseTabButton.qml` - Individual tabs with status indicators
- **Prompts Tab:** `bridges/assets/qml/PromptsTab.qml` - AI conversation interface
...
- **Pure-Rust audio stack** (replaced Qt Multimedia / FFmpeg for 16 KB Android
  ...
  - **QML:** `bridges/assets/qml/RecordingPlaybackItem.qml` — recording/playback UI (no `QtMultimedia`).
...
2. **Export User Data:** `backend/src/app_data.rs` - `export_user_data_to_assets()`
   ...
   - Exports user-imported dictionaries to `user_dictionaries.sqlite3` (`export_user_dictionaries()`): `dictionaries`, `dict_words` and `dict_resources` (the `res/` CSS/JS/images), restored with re-keyed ids
...
5. **Download New Databases:** `bridges/assets/qml/DownloadAppdataWindow.qml`
...
Users import, rename, and delete their own StarDict/GoldenDict dictionaries from `bridges/assets/qml/DictionariesWindow.qml` (launched via `cpp/dictionaries_window.cpp`). The bridge is `bridges/src/dictionary_manager.rs` (`DictionaryManager`, registered as a QmlModule; qmllint stub `bridges/assets/qml/com/profoundlabs/simsapa/DictionaryManager.qml`); mutating ops route through `backend/src/dictionary_manager_core.rs`, which holds the global `DICT_MGR_LOCK`. PRD: [tasks/prd-dictionaries-window-progress-frames.md](tasks/prd-dictionaries-window-progress-frames.md).
...
- **Delete:** single `DELETE FROM dictionaries WHERE id = ?` relying on the `dict_words.dictionary_id` FK `ON DELETE CASCADE` (migration `…/2025-05-03-143320_create-tables/up.sql:42`). `count_words_for_dictionary` is read before the delete to report `removed_count`. Indeterminate progress bar, no abort. A pending upgrade snapshot (`import-me/user_dictionaries.sqlite3`) is pruned of the label, its words and its resources too (`app_data::remove_label_from_user_dictionaries_snapshot`, file removed once empty), otherwise the startup restore would re-create the dictionary.
...
- **Rename:** `DictionaryEditDialog` emits `rename_requested(dictionary_id, old_label, new_label)` (no direct bridge call); the window switches to Idx 3 and calls `rename_label`. `rename_user_dictionary` sets `indexed_at = NULL` so the next reconcile re-indexes. The rename SQL is `db::dictionaries::rename_dictionary_label_in(conn, old, new)`, applied to the live DB and to a pending upgrade snapshot (`app_data::rename_label_in_user_dictionaries_snapshot`).
- **Startup reconcile:** `start_reconcile()` drives `bridges/assets/qml/DictionaryIndexProgressWindow.qml` (shown by `cpp/gui.cpp` before `SuttaSearchWindow`) via `reconcileProgress(stage, done, total)` / `reconcileFinished()`. A pending `import-me/user_dictionaries.sqlite3` snapshot makes `reconcile_needed()` true and is restored first; a snapshot dictionary whose label already exists in the live DB is skipped together with its words, so the snapshot is always consumed (`RestoringDictionaries` → `"Restoring imported dictionaries: <done>/<total> words"`). `reconcile_progress_to_signal` formats `IndexingDictionary` as `"Indexing: <i>/<n> <label>, <done>/<total> words"`. Indexing ticks every 1000 words (`backend/src/search/indexer.rs:715`); orphaned Tantivy entries from deleted dictionaries are cleaned by the `DroppingOrphans` pass. Reconcile is not cancellable.
