...
- **Key Components:**
  ...
  - `window_manager.cpp/.h` - Multiple window management system. **Closing a window only hides it** — it stays in its `WindowManager` list so the next open can revive it instead of loading another `QQmlApplicationEngine`, so `window_is_open()` (the root's `visible` property), not list membership, is what distinguishes an open window from a pooled one. `create_sutta_search_window()` revives the newest pooled window (`clear_all_tabs()`, keep the `window_id`, move to the end of the list, `show_and_activate_window()`); the `window_id`-less dispatch fallbacks must use `last_open_sutta_search_window()` / `first_open_sutta_search_window()`, never bare `last()`/`first()`, or they re-show a window the user closed. Session save (`gui.cpp` `aboutToQuit`) filters on `visible` for the same reason. See [docs/window-lifecycle-and-reuse.md](./docs/window-lifecycle-and-reuse.md)
...
### UI Components
- **Main Windows:** `cpp/window_manager.cpp`, QML window components — lifecycle (closed = hidden + pooled, revive on open, `visible`-filtered dispatch and session save): [docs/window-lifecycle-and-reuse.md](./docs/window-lifecycle-and-reuse.md)
