...
// `remote = "Self"` makes the derives generate inherent `serialize` /
// `deserialize` functions instead of the trait impls, so the hand-written impls
// below can wrap them. Deserializing must not be bypassable: `AppSettings` is
// read back in three places (`db/appdata.rs`, `db/mod.rs`, `app_data.rs`), and a
// migration hook at any one call site would silently skip the others.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(default)]
#[serde(remote = "Self")]
pub struct AppSettings {
    ...
    /// Capture of the removed `mobile_top_bar_margin` setting, read from older
    /// stored settings only. `Deserialize` carries the value over into
    /// `mobile_extra_top_margin` and clears this, and it is never written back.
    #[serde(rename = "mobile_top_bar_margin", skip_serializing)]
    pub(crate) legacy_mobile_top_bar_margin: Option<MobileTopBarMargin>,
...
impl Serialize for AppSettings {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        // The inherent fn generated by the derive, not this trait method.
        AppSettings::serialize(self, serializer)
    }
}
...
impl<'de> Deserialize<'de> for AppSettings {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        // The inherent fn generated by the derive, not this trait method.
        let mut settings = AppSettings::deserialize(deserializer)?;
        settings.migrate_mobile_top_bar_margin();
        Ok(settings)
    }
}

impl AppSettings {
    /// Carry the removed `mobile_top_bar_margin` setting over to
    /// `mobile_extra_top_margin`: `SystemValue` -> 0 (Qt supplies the inset),
    /// `CustomValue(v)` -> v (the user keeps the spacing they chose).
    ///
    /// The container-level `#[serde(default)]` makes an absent key and an
    /// explicit `0` indistinguishable, so the carry-over is applied whenever the
    /// new value is `0` — which is the same result either way, since
    /// `SystemValue` migrates to `0`.
    fn migrate_mobile_top_bar_margin(&mut self) {
        if let Some(legacy) = self.legacy_mobile_top_bar_margin.take() {
            if self.mobile_extra_top_margin == 0 {
                self.mobile_extra_top_margin = match legacy {
...
<tr>
    ...
    <td class="constr-desc">{dpd.construction}</td>
            ...
            mobile_extra_top_margin: 0,
            legacy_mobile_top_bar_margin: None,
...
/// Removed setting, kept only to read the value out of older stored settings.
/// Replaced by `AppSettings::mobile_extra_top_margin`; see the `Deserialize`
/// impl for `AppSettings` and docs/android-edge-to-edge-and-safe-areas.md
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[derive(Default)]
pub(crate) enum MobileTopBarMargin {
    #[default]
    SystemValue,
    CustomValue(u32),
}
...
mod tests {
    ...
    /// The removed `mobile_top_bar_margin` setting is carried over to
    /// `mobile_extra_top_margin` inside `Deserialize`, so every reader of stored
    /// settings gets it. See docs/android-edge-to-edge-and-safe-areas.md
    #[test]
    fn legacy_system_value_margin_becomes_zero() {
        let settings: AppSettings =
            serde_json::from_str(r#"{"mobile_top_bar_margin": "SystemValue"}"#).unwrap();
        assert_eq!(settings.mobile_extra_top_margin, 0);
    }

    #[test]
    fn legacy_custom_value_margin_is_carried_over() {
        let settings: AppSettings =
            serde_json::from_str(r#"{"mobile_top_bar_margin": {"CustomValue": 24}}"#).unwrap();
        assert_eq!(settings.mobile_extra_top_margin, 24);
    }

    #[test]
    fn new_margin_key_is_left_alone() {
        let settings: AppSettings =
            serde_json::from_str(r#"{"mobile_extra_top_margin": 8}"#).unwrap();
        assert_eq!(settings.mobile_extra_top_margin, 8);
    }

    #[test]
    fn settings_without_either_margin_key_default_to_zero() {
        let settings: AppSettings = serde_json::from_str(r#"{"sutta_font_size": 20}"#).unwrap();
        assert_eq!(settings.mobile_extra_top_margin, 0);
        assert_eq!(settings.sutta_font_size, 20);
    }

    /// The legacy key must not be written back out, or it would be re-read and
    /// re-migrated on every save.
    #[test]
    fn migrated_settings_do_not_re_serialize_the_legacy_key() {
        let settings: AppSettings =
            serde_json::from_str(r#"{"mobile_top_bar_margin": {"CustomValue": 24}}"#).unwrap();
        let json = serde_json::to_string(&settings).unwrap();
        assert!(!json.contains("mobile_top_bar_margin"), "{}", json);
        assert!(json.contains("\"mobile_extra_top_margin\":24"), "{}", json);

        let round_tripped: AppSettings = serde_json::from_str(&json).unwrap();
        assert_eq!(round_tripped.mobile_extra_top_margin, 24);
        assert_eq!(round_tripped, settings);
    }

    /// A blob written by a new build but still carrying the old key (e.g. an
    /// exported `app_settings.json` merged by hand): the new value wins.
    #[test]
    fn new_margin_key_wins_over_the_legacy_one() {
        let settings: AppSettings = serde_json::from_str(
            r#"{"mobile_extra_top_margin": 10, "mobile_top_bar_margin": {"CustomValue": 24}}"#,
        )
        .unwrap();
        assert_eq!(settings.mobile_extra_top_margin, 10);
        assert_eq!(settings.legacy_mobile_top_bar_margin, None);
