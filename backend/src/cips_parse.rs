...
//! into the topic index data structures.
//!
//! The CSV is tab-delimited with 3 columns: headword, subheading, locator
//!
//! Two callers share this module: the CLI's `parse-cips-index` bootstrap
//! command, which writes `assets/general-index.json`, and the in-app update
//! that fetches the CSV and stores the result in the database. Nothing here
//! touches the filesystem or prints — the entry points take the CSV as a string
//! (or read a path in a thin wrapper) and **return** their diagnostics in
//! `CipsParseOutcome::warnings`, so both callers can present them their own way.
...
use crate::topic_index::{TopicIndexEntry, TopicIndexHeadword, TopicIndexLetter, TopicIndexRef};

/// The parsed index together with the diagnostics gathered on the way.
///
/// The warnings are *returned* rather than printed: the CLI prints them, the
/// app shows them in its update report. They are advisory — a warning never
/// aborts a parse.
...
/// Scan a CIPS general-index.csv (tab-delimited) into rows.
///
/// Returns the rows **and** the malformed-line warnings rather than printing
/// them: this runs on-device during an in-app update, where there is no console
/// to print to. The warning text is unchanged from when it was an `eprintln!`,
/// minus the `Warning: ` prefix the caller adds.
fn parse_csv_str(csv: &str) -> (Vec<CsvRow>, Vec<String>) {
    ...
    for (line_num, line) in csv.lines().enumerate() {
        ...
        if parts.len() < 3 {
            warnings.push(format!("Line {} has fewer than 3 columns: {:?}", line_num + 1, line));
            continue;
        }
...
/// Returns: "dhp33-43"
///
/// Malformed locators are pushed onto `warnings` instead of being printed. The
/// sink is threaded down from `build()` because this is called from deep inside
/// the row loop; there is no single boundary to return them from.
fn parse_custom_locator(locator: &str, warnings: &mut Vec<String>) -> Option<String> {
    ...
    if parts.len() < 4 {
        warnings.push(format!("Invalid CUSTOM format (not enough parts): {}", locator));
        return None;
    }
    ...
    if path.is_empty() {
        warnings.push(format!("Could not extract sutta ref from CUSTOM URL: {}", locator));
        return None;
    }
...
fn parse_sutta_ref(locator: &str, warnings: &mut Vec<String>) -> String {
    // Check for CUSTOM format first
    if is_custom_format(locator)
        && let Some(sutta_ref) = parse_custom_locator(locator, warnings) {
...
impl IndexBuilder {
    ...
    /// `warnings` collects the malformed-locator diagnostics raised while
    /// parsing the locators of each sub-entry.
    fn build<F>(self, title_lookup: F, warnings: &mut Vec<String>) -> Vec<TopicIndexLetter>
    where
            ...
            for headword in headword_keys {
                ...
                for sub in sub_keys {
                    ...
                    for locator in sorted_locators {
                        let sutta_ref = parse_sutta_ref(&locator, warnings);
...
/// Parse CIPS general-index.csv **content** and return the topic index data
/// structure together with the diagnostics gathered on the way.
///
/// This is the entry point the in-app update uses: it never touches the
/// filesystem, so the CSV can come straight from an HTTP response body.
///
/// # Arguments
/// * `csv` - The CSV content, tab-delimited with 3 columns
...
where
    F: Fn(&str) -> Option<String>,
{
    let (rows, mut warnings) = parse_csv_str(csv);
    ...
    // The CSV-scan warnings come first, then the locator warnings raised during
    // the build — the same relative order they were printed in when both were
    // `eprintln!`s.
    let letters = builder.build(title_lookup, &mut warnings);

    Ok(CipsParseOutcome { letters, warnings })
}

/// Parse a CIPS general-index.csv file and return the topic index data structure.
///
/// A thin wrapper over `parse_cips_index_str()` that reads the file; the CLI
/// bootstrap command uses this form.
///
/// # Arguments
    ...
    F: Fn(&str) -> Option<String>,
{
    ...
    parse_cips_index_str(&content, title_lookup)
}
...
mod tests {
    ...
    // The two locator parsers take a warnings sink now that their `eprintln!`s
    // are returned diagnostics. These shadow them with the pure form, so every
    // test below reads exactly as it did before the sink was threaded through
    // — the sink is exercised separately by
    // `test_parse_custom_locator_returns_its_warnings`.
    fn parse_custom_locator(locator: &str) -> Option<String> {
        super::parse_custom_locator(locator, &mut Vec::new())
    }

    fn parse_sutta_ref(locator: &str) -> String {
        super::parse_sutta_ref(locator, &mut Vec::new())
    }
    ...
    #[test]
    fn test_parse_csv_str_returns_malformed_line_warnings() {
        // Blank lines are skipped silently; a short line is reported and
        // skipped. The line number is 1-based and counts every line, including
        // the blank one.
        let csv = "good\tsub\tDN1\n\nshort line\ngood2\tsub\tMN1\n";
        let (rows, warnings) = parse_csv_str(csv);

        assert_eq!(rows.len(), 2);
        assert_eq!(rows[0].headword, "good");
        assert_eq!(rows[1].headword, "good2");
        assert_eq!(
            warnings,
            vec![r#"Line 3 has fewer than 3 columns: "short line""#]
        );
    }

    #[test]
    fn test_parse_custom_locator_returns_its_warnings() {
        let mut warnings = Vec::new();

        // Too few parts.
        assert_eq!(
            super::parse_custom_locator("CUSTOM:Dhp:Title", &mut warnings),
            None
        );
        // A URL with no path component after the domain.
        assert_eq!(
            super::parse_custom_locator("CUSTOM:Dhp:Title:suttacentral.net/", &mut warnings),
            None
        );
        // A well-formed one adds nothing.
        assert_eq!(
            super::parse_custom_locator(
                "CUSTOM:Dhp:Dhp Chapter 3:suttacentral.net/dhp33-43/en/sujato",
                &mut warnings
            ),
            Some("dhp33-43".to_string())
        ...
        assert_eq!(
            warnings,
            vec![
                "Invalid CUSTOM format (not enough parts): CUSTOM:Dhp:Title",
                "Could not extract sutta ref from CUSTOM URL: CUSTOM:Dhp:Title:suttacentral.net/",
            ]
        );
    }

    #[test]
    fn test_parse_cips_index_str_merges_both_warning_sources() {
        // One malformed CSV line and one malformed CUSTOM locator: the
        // CSV-scan warning comes first, the locator warning second.
        let csv = "short line\nheadword\tsub\tCUSTOM:Dhp:Title\n";
        let outcome = parse_cips_index_str(csv, |_uid| None).unwrap();

        assert_eq!(
            outcome.warnings,
            vec![
                r#"Line 1 has fewer than 3 columns: "short line""#,
                "Invalid CUSTOM format (not enough parts): CUSTOM:Dhp:Title",
            ]
        );
        assert_eq!(outcome.letters.len(), 1);
        assert_eq!(outcome.letters[0].headwords.len(), 1);
    }
