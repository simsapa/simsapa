...
- [ ] 1.0 Rust bridge: `request_id` plumbing and per-request cancellation (FR-C1, FR-C3 engine side, FR-D4)
  - [ ] 1.1 In `bridges/src/prompt_manager.rs`, add a `request_id: &QString` parameter to `prompt_request`, `prompt_request_with_messages`, `sequential_prompt_request`, and `sequential_prompt_request_with_messages`; thread the owned `String` into each spawned worker.
