...
- [x] 5.1 Re-run the DPD bootstrap step (or the minimal path that invokes `convert_dpd_epd_word_links`) to regenerate `dictionaries.sqlite3` in the dist assets dir. — Done by user (re-bootstrap complete).
- [x] 5.2 Verify the DB: query `SELECT definition_html FROM dict_words WHERE uid='happy/dpd'` and confirm epd words are now `<a class="epd word_link" href="ssp://word_lookup/...">`; confirm `definition_plain` for the same row still contains the plain word text and no `<a` markup; sanity-check the total rewritten row count is plausible. — Confirmed: 77,850 dpd rows carry `ssp://word_lookup/` anchors, 0 bare `<b class=epd>` remain, 0 `definition_plain` leakage; `happy/dpd` plain text keeps the diacritic words.
- [x] 5.3 Fetch `GET /get_word_html_by_uid/window_0/happy/dpd` from the live API and confirm the rendered HTML carries the new links + class. — Confirmed: 28 `<a class="epd word_link" href="ssp://word_lookup/...">` anchors rendered (diacritics percent-encoded).
- [x] 5.4 Confirm the `POST /word_lookup` route responds `200` for `{ window_id, query }` (curl the live API). — Confirmed HTTP 200 for `{window_id:"window_0", query:"attamana"}`.
- [x] 5.5 Manual UX check (user-driven, since GUI testing is not suitable for the agent): clicking a word on `happy/dpd` reveals the sidebar and runs a Combined lookup; a single-entry word shows its entry, a multi-entry word shows several, styling is distinct. — **Confirmed by user.** Styling refined during the check: `a.word_link` uses a dashed `border-bottom` (color `--primary-alt`, matching the `.sutta-link` look) that becomes solid on hover, with `p:has(a.word_link) { line-height: 200% }` for row spacing. **Gotcha:** the anchor also carries the pre-existing `epd` class, whose `.epd:hover { text-decoration: underline }` (specificity 0,2,0) overrode the hover underline; `a.word_link:hover` must set `text-decoration: none` explicitly (specificity 0,2,1) to win.
- [x] 5.6 Run `cd backend && cargo test` and `make build -B` for a final clean state; update `PROJECT_MAP.md` / relevant `docs/` if warranted. — Transform test passes, build clean, PROJECT_MAP.md updated with the EPD word-lookup feature entry.
